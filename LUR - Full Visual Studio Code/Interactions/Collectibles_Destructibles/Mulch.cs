...
//README: A simple class that uses the MulchType enum to set the effects of the Mulch piles the player can interact with.
//These interactions will change world conditions depending on what level the mulch is located in, such as changing character dialogue or opening the boss area in the game's hub.
...
public class Mulch : Interactable
{
    ...
    //The switch statement within Interact() determines which global variables within GameController get flipped, which has an effect on world events.

    //For example, if the Mulch in game has its MulchType marked as ANGEL, this will allow the player to talk to the Mulchant character to awaken the 

    //ancient Tree in the game's hub area.
