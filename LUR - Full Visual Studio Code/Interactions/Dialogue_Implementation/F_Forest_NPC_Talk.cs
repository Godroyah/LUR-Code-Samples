...
//README: This class is one example of how characters in the gameworld are linked up with their correct dialogue. There is one like this for each level.
//An enum like the one above is used to list the characters present in the level; scroll down to Interact() for more.
...
public class F_Forest_NPC_Talk : Interactable
{
    ...
    //Interact() uses a switch statement to activate a Coroutine specific to the character in question.
    //In each Coroutine, a character's dialogue may change if you are:
    // - Speaking to them for the first time
    // - Depending on world conditions dictated by the GameController
    // - A random number generator which chooses from a number of flavor dialogue options.
